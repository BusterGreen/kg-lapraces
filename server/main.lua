...
local function IsWhitelisted(CitizenId)
    ...
    local Perms = KGCore.Functions.GetPermission(Player.PlayerData.source)
...
RegisterNetEvent('kg-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    local src = source
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('kg-phone:client:RaceNotify', src, 'You have won the WR from ' .. RaceData.RaceName ..
    ...
    TriggerClientEvent('kg-lapraces:client:PlayerFinishs', -1, RaceData.RaceId, PlayersFinished, Player)
    ...
    TriggerClientEvent('kg-phone:client:UpdateLapraces', -1)
end)

RegisterNetEvent('kg-lapraces:server:CreateLapRace', function(RaceName)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        if IsNameAvailable(RaceName) then
            TriggerClientEvent('kg-lapraces:client:StartRaceEditor', source, RaceName)
        else
            TriggerClientEvent('KGCore:Notify', source, 'There is already a race with this name.', 'error')
        end
    else
        TriggerClientEvent('KGCore:Notify', source, 'You have not been authorized to race\'s to create.', 'error')
    end
end)

RegisterNetEvent('kg-lapraces:server:JoinRace', function(RaceData)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('KGCore:Notify', src, 'You were the only one in the race, the race had ended', 'error')
            TriggerClientEvent('kg-lapraces:client:LeaveRace', src, Races[CurrentRace])
        else
    ...
    TriggerClientEvent('kg-lapraces:client:JoinRace', src, Races[RaceId], AvailableRaces[AvailableKey].Laps)
    TriggerClientEvent('kg-phone:client:UpdateLapraces', -1)
    local creatorsource = KGCore.Functions.GetPlayerByCitizenId(AvailableRaces[AvailableKey].SetupCitizenId).PlayerData
                              .source
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('kg-phone:client:RaceNotify', creatorsource,
            string.sub(Player.PlayerData.charinfo.firstname, 1, 1) .. '. ' .. Player.PlayerData.charinfo.lastname ..
                ' the race has been joined!')
    end
end)

RegisterNetEvent('kg-lapraces:server:LeaveRace', function(RaceData)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    local RaceName
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('KGCore:Notify', src, 'You were the only one in the race.The race had ended.', 'error')
        TriggerClientEvent('kg-lapraces:client:LeaveRace', src, Races[RaceId])
...
RegisterNetEvent('kg-lapraces:server:SetupRace', function(RaceId, Laps)
            ...
            if not Races[RaceId].Started then
                ...
                SetTimeout(5 * 60 * 1000, function()
                    if Races[RaceId].Waiting then
                        local AvailableKey = GetOpenedRaceKey(RaceId)
                        for cid, _ in pairs(Races[RaceId].Racers) do
                            local RacerData = KGCore.Functions.GetPlayerByCitizenId(cid)
                            if RacerData ~= nil then
                                TriggerClientEvent('kg-lapraces:client:LeaveRace', RacerData.PlayerData.source,
                                    Races[RaceId])
                            end
                        end
                        table.remove(AvailableRaces, AvailableKey)
                ...
                TriggerClientEvent('KGCore:Notify', source, 'The race is already running', 'error')
            end
        else
            TriggerClientEvent('KGCore:Notify', source, 'The race is already running', 'error')
        end
    else
        TriggerClientEvent('KGCore:Notify', source, 'This race does not exist :(', 'error')
    end
end)

RegisterNetEvent('kg-lapraces:server:CancelRace', function(raceId)
    local src = source
    local Player = KGCore.Functions.GetPlayer(source)
    local AvailableKey = GetOpenedRaceKey(raceId)

    TriggerClientEvent('KGCore:Notify', src, 'Stopping the race: ' .. raceId, 'error')
    ...
    if AvailableKey ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == Player.PlayerData.citizenid then
            for cid, _ in pairs(Races[raceId].Racers) do
                local RacerData = KGCore.Functions.GetPlayerByCitizenId(cid)
                if RacerData ~= nil then
                    TriggerClientEvent('kg-lapraces:client:LeaveRace', RacerData.PlayerData.source, Races[raceId])
                end
            end
        ...
        TriggerClientEvent('KGCore:Notify', src, 'Race not open: ' .. raceId, 'error')
    end
end)

RegisterNetEvent('kg-lapraces:server:UpdateRaceState', function(RaceId, Started, Waiting)
    Races[RaceId].Waiting = Waiting
    Races[RaceId].Started = Started
end)

RegisterNetEvent('kg-lapraces:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('kg-lapraces:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId])
end)

RegisterNetEvent('kg-lapraces:server:StartRace', function(RaceId)
    local src = source
    local MyPlayer = KGCore.Functions.GetPlayer(src)
    local AvailableKey = GetOpenedRaceKey(RaceId)
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == MyPlayer.PlayerData.citizenid then
            ...
            for CitizenId, _ in pairs(Races[RaceId].Racers) do
                local Player = KGCore.Functions.GetPlayerByCitizenId(CitizenId)
                if Player ~= nil then
                    TriggerClientEvent('kg-lapraces:client:RaceCountdown', Player.PlayerData.source)
                end
            end
            TriggerClientEvent('kg-phone:client:UpdateLapraces', -1)
        else
            TriggerClientEvent('KGCore:Notify', src, 'You are not the creator of the race..', 'error')
        end
    else
...
RegisterNetEvent('kg-lapraces:server:SaveRace', function(RaceData)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
...
KGCore.Functions.CreateCallback('kg-lapraces:server:GetRacingLeaderboards', function(_, cb)
    cb(Races)
end)

KGCore.Functions.CreateCallback('kg-lapraces:server:GetRaces', function(_, cb)
    cb(AvailableRaces)
end)

KGCore.Functions.CreateCallback('kg-lapraces:server:GetListedRaces', function(_, cb)
    cb(Races)
end)

KGCore.Functions.CreateCallback('kg-lapraces:server:GetRacingData', function(_, cb, RaceId)
    cb(Races[RaceId])
end)

KGCore.Functions.CreateCallback('kg-lapraces:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(KGCore.Functions.GetPlayer(source).PlayerData.citizenid))
end)

KGCore.Functions.CreateCallback('kg-lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
    cb(IsWhitelisted(KGCore.Functions.GetPlayer(source).PlayerData.citizenid), IsNameAvailable(TrackName))
end)

KGCore.Functions.CreateCallback('kg-lapraces:server:CanRaceSetup', function(_, cb)
    cb(Config.RaceSetupAllowed)
end)

KGCore.Functions.CreateCallback('kg-lapraces:server:GetTrackData', function(_, cb, RaceId)
...
KGCore.Commands.Add("cancelrace", "Cancel going race..", {}, false, function(source, args)
    local Player = KGCore.Functions.GetPlayer(source)
    ...
    if IsWhitelisted(Player.PlayerData.citizenid) then
        ...
        if RaceName ~= nil then
            ...
            if Races[RaceId].Started then
                ...
                TriggerClientEvent('KGCore:Notify', source, 'This race has not started yet.', 'error')
            end
        end
    else
        TriggerClientEvent('KGCore:Notify', source, 'You have not been authorized to do this.', 'error')
    end
end)

KGCore.Commands.Add("togglesetup", "Turn on / off racing setup", {}, false, function(source, _)
    local Player = KGCore.Functions.GetPlayer(source)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        Config.RaceSetupAllowed = not Config.RaceSetupAllowed
        if not Config.RaceSetupAllowed then
            TriggerClientEvent('KGCore:Notify', source, 'No more races can be created!', 'error')
        else
            TriggerClientEvent('KGCore:Notify', source, 'Races can be created again!', 'success')
        end
    else
        TriggerClientEvent('KGCore:Notify', source, 'You have not been authorized to do this.', 'error')
    end
